...
export default function App() {
  ...
  function toggleLike(post) {
    ...
    setPostsByDate((prev) => {
      const arr = (prev[post.date] || []).map((p) => {
  ...
  function deletePost(post) {
    if (!isAdmin) return;
    setPostsByDate((prev) => {
      const arr = (prev[post.date] || []).filter((p) => p.id !== post.id);
      return { ...prev, [post.date]: arr };
    });
  }
          ...
          <div className="bg-white text-black w-full max-w-2xl rounded-2xl shadow-xl p-6 relative">
            ...
            <div className="flex flex-col gap-3">
              <input
                ...
                onFocus={() => setPhName("")}
                onBlur={() => !nameInput && setPhName("사용자 이름")}
                placeholder={phName}
                className={`w-full border rounded-xl px-4 py-3 ${invalidName ? "border-red-500" : "border-gray-300"}`}
              />
              <input
                value={titleInput}
                onChange={(e) => setTitleInput(e.target.value)}
                onFocus={() => setPhTitle("")}
                onBlur={() => !titleInput && setPhTitle("제목")}
                placeholder={phTitle}
                className={`w-full border rounded-xl px-4 py-3 ${invalidTitle ? "border-red-500" : "border-gray-300"}`}
              />
              <textarea
                value={bodyInput}
                onChange={(e) => setBodyInput(e.target.value)}
                onFocus={() => setPhBody("")}
                onBlur={() => !bodyInput && setPhBody("환상적인 글을 적어보세요!")}
                placeholder={phBody}
                rows={8}
              ...
              {attachments.length > 0 && (
                <div className="grid grid-cols-3 gap-2">
                  {attachments.map((src, i) => (
                    <img key={i} src={src} alt={`attachment-${i}`} className="w-full h-24 object-cover rounded-lg border" />
                  ))}
                </div>
              )}
        ...
        <div className="fixed inset-0 z-[70] bg-black/70 flex items-center justify-center p-4">
          <div className="bg-white text-black w-full max-w-3xl rounded-2xl shadow-xl p-6 relative">
            ...
            {viewPost.images?.length > 0 && (
              <div className="mt-4 grid grid-cols-2 gap-3">
                {viewPost.images.map((src, i) => (
                  <img key={i} src={src} alt={`img-${i}`} className="w-full rounded-xl border" />
                ))}
              </div>
            )}
            ...
            <div className="mt-5 flex items-center gap-2">
              ...
              {isAdmin && (
                <button onClick={() => { deletePost(viewPost); setViewPost(null); }} className="ml-auto text-white text-sm bg-red-600 px-3 py-1 rounded">삭제</button>
              )}
            </div>
          </div>
        </div>
